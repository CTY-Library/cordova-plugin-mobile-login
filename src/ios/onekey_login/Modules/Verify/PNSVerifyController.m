...
@property (nonatomic, assign) BOOL isCanUseVerify;
...
- (void)checkAndPrepareEnv {
    ...
    self.isCanUseVerify = YES;
    
    //__weak typeof(self) weakSelf = self;
    //环境检查，异步返回
    [[TXCommonHandler sharedInstance] checkEnvAvailableWithAuthType:PNSAuthTypeVerifyToken
                                                           complete:^(NSDictionary * _Nullable resultDic) {
        NSLog(@"环境检查返回：%@", resultDic);
        //weakSelf.isCanUseVerify = [PNSCodeSuccess isEqualToString:[resultDic objectForKey:@"resultCode"]];
        
//        if (weakSelf.isCanUseVerify == YES) {
...
//    }
//
//    if (!self.isCanUseVerify) {
//        MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
//        hud.mode = MBProgressHUDModeText;
//        hud.label.text = @"当前环境不支持本机号码校验";
//        [hud hideAnimated:YES afterDelay:2.0];
//        return;
//    }
//
   ...
   // __weak typeof(self) weakSelf = self;
    [[TXCommonHandler sharedInstance] getVerifyTokenWithTimeout:3.0
        ...
        if ([PNSCodeSuccess isEqualToString:[resultDic objectForKey:@"resultCode"]]) {
            ...
            NSLog(@"接下来可以拿着Token和用户输入手机号去服务端进行校验，SDK提供服务到此结束");
